...
from nltk.tag import pos_tag  # Import pos_tag for lemmatization context
...
def process_text(text, task):
    ...
    try:
        if task == "tokenize":
            ...
            # Use NLTK for POS tagging for lemmatization
            try:
            ...
            if nlp:
                doc = nlp(text)
                pos_tags = [(token.text, token.tag_) for token in doc]  # Use spaCy for POS tagging
                return pos_tags
            else:
                # Fallback to NLTK's pos_tag if spaCy not loaded
                try:
